...
export const staffApi = {
  ...
  pickupOrders: async () => {
    const response = await request<DeliveryTask[]>('/staff/pickup-orders');
    return (response.data || []).map(normalizeTask);
  },
  sendPickupOtp: async (id: string) => {
    const response = await request<{ success: boolean; message: string }>(`/staff/pickup-orders/${id}/send-otp`, {
      method: 'POST',
    });
    return response.data;
  },
  verifyPickup: async (id: string, payload: { otp: string }) => {
    const response = await request<DeliveryTask>(`/staff/pickup-orders/${id}/verify-pickup`, {
      method: 'POST',
      body: JSON.stringify(payload),
    });
    return normalizeTask(response.data);
